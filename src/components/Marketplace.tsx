      ...
      <div className="p-4 border-b border-[var(--color-border)] bg-[var(--color-card)]/50">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#7C5CFF] hover:bg-[#6D4CFF] text-white text-xs font-bold shadow-md shadow-[#7C5CFF]/25 transition-all"
          >
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-medium whitespace-nowrap transition-colors ${
                selectedCategory === cat.key
                  ? 'text-[#7C5CFF] bg-[#7C5CFF]/10 font-bold'
        ...
        {filteredListings.length > 0 ? (
          filteredListings.map(item => (
            <div
              ...
              className="group rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] overflow-hidden cursor-pointer hover:border-[#7C5CFF] shadow-sm hover:shadow-xl hover:shadow-[#7C5CFF]/10 transition-all duration-300 flex flex-col justify-between"
            >
              <div>
                {/* Image & Badges */}
                <div className="relative aspect-video overflow-hidden bg-black/40">
                  ...
                  <div className="absolute top-2.5 left-2.5 flex gap-1.5">
                    {item.type === 'trade' ? (
                      <span className="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-[#2DD4BF] text-slate-950 shadow-md">
                        {t('tradeBadge')}
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-[#7C5CFF] text-white shadow-md">
                        ${item.price}
                      </span>
                    )}
                ...
                {/* Listing Details */}
                <div className="p-3">
                  <span className="text-[10px] font-bold text-[#2DD4BF] uppercase tracking-wider block mb-0.5">
                    {item.category}
                  </span>
                  <h3 className="font-bold text-sm text-[var(--color-text-primary)] line-clamp-2 leading-snug group-hover:text-[#7C5CFF] transition-colors mb-1">
                    {item.title}
                  </h3>
